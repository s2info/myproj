...
.result_vulnerable {
 ...
 $(document).ready(function () {	
	...
	$.sdPopup = function(){
		$("#popuplayer").bPopup({
			content : 'iframe',
			iframeAttr : ('scolling="yes" frameborder="10" width="100%" height="100%"'),
			contentContainer : '.popupContent',
			loadUrl:"/securityDay/popSecurityDay.do"
		});
	}
   ...
   function searchList(selectdiv)
   {
	    selectdiv.siblings().removeClass('on');
	   	...
	   	var search_date = $('#search_date').val();//selectdiv.attr('search_date');
...
console.log("ddd");
			 var y = (x+nameOfCookie.length);
			 console.log(y);
			 console.log(document.cookie.substring(x,y));
			 if(document.cookie.substring(x,y) == nameOfCookie){
				console.log("xxx");				 
				 if((endOfCookie=document.cookie.indexOf(";",y)) == -1){
					 endOfCookie = document.cookie.length;
	...
	$(".ui-datepicker-trigger").css("margin","0px 0 -9px 5px");
	
	$.searchRefrash = function(){
		var firstdiv = $('.result_list').find('div').eq(0);
	   	//$('#majrCode').val(firstdiv.attr('diagcode'));
		//$('#buseoIndc').val(firstdiv.attr('buseoindc'));
		//$('#majrName').val(firstdiv.attr('diagDesc'));
...
<div id="container">
    ...
    <div class="snb">
        <!-- S : socre -->
        <div class="block1">
            <li class="blockcell1"><img style="width:26px;height:26px;" src="/img/icon_user_2.png" /></li>
        ...
        <c:forEach var="result" items="${policyScoreList}" varStatus="status">
        	<c:if test="${result.score != '-999' && result.count != '-'}" >
        		<div style="${auth ne '3' ? 'cursor:pointer;' : '' }" class="graph4_date5 ${auth ne '3' ? 'diag_majr' : '' }" search_date="${result.search_rgdt_date }" org_code="${result.org_code }" diagcode="${result.diag_majr_code}" diagDesc="${result.diag_desc}" buseoindc="${result.buseo_indc}" is_sub_org="${result.is_sub_org }">
    ...
    <div id="content">
        	...
        	<div class="main_blockcell1">
                <div class="index_block1_1">
                    <div class="graph2_block1">
                     <c:choose>
	                    <c:when test="${totalUserInfo.score == '-999'}">
                    ...
                    <c:if test="${totalUserInfo.iscollabor eq 'N' }" >
                    <div class="graph2_block1">
        ...
        <div class="main_block2">
            <div class="index_block3cell">
            	...
            	<li class="cell2 result_status">전체 정책 <span class="txcell1">양호0개</span> <span class="txcell2">취약0개</span></li>
            	<li class="cell2">수집날짜  <input type="text" id="search_date" name="search_date" value="${totalUserInfo.day_val }" readonly style="width:100px;z-indx:100000" class="srh" onchange="javascript:$.searchRefrash()"/>&nbsp;&nbsp;</li>
            </div>
    ...
    <div class="aside">
    	<ul>
        	<li><a href="/man/noticelist.do"><img src="/img/icon_right_1.png" /><span style="color:#fff;">공지사항</span><c:if test="${readcntInfo.noticnt > 0 }"><span class="icon_new"></span></c:if></a></li>
            <li><a href="/man/qnaList.do"><img src="/img/icon_right_2.png" /><span style="color:#fff;">Q&amp;A</span><c:if test="${readcntInfo.qnacnt > 0 }"><span class="icon_new"></span></c:if></a></li>
            <li><a href="/man/faqlist.do"><img src="/img/icon_right_3.png" /><span style="color:#fff;">FAQ</span><c:if test="${readcntInfo.faqcnt > 0 }"><span class="icon_new"></span></c:if></a></li>
        <c:if test="${sdUseYn.resultYn eq 'Y' }">    
			<li class="cell1" <c:if test="${sdUseYn.checkYn eq 'N'}">  onclick="$.sdPopup();"  </c:if> style="height:90px; background: ${sdUseYn.checkYn eq 'Y' ? '#047700' : '#434343; cursor: pointer;' }"><img src="/img/icon_right_4.png" />
			Security<br/>Day
			 <c:if test="${sdUseYn.checkYn eq 'Y' }">  
			 <br/>(완료) 
			 </c:if> 
			</li>
		</c:if>
