...
$(function () {
	...
	$.text_box = function(){
		...
		var $this =$("#questionText");
		var t = $this.text();
		console.log("text == " + t);
		$("#questionText").html(t.replace('&lt;','<').replace('&gt;','>').replace('quot;','"'));
	};
	...
	$.answer_ch = function(){
		...
		if($("#fileYn").val()=="Y" ){
		...
		}else if($("#question").val() != "" && $("#answerType").val()!=""){
			var answer ="";
			//alert("2");
			if($("#answerType").val()=="2"){
				answer = $('input:checkbox[name=answerMc]:checked').val();
				var checked_length =  $('input:checkbox[name=answerMc]:checked').length;
				if(checked_length <= 0){
					alert('답을 선택하세요.');
					return false;
				}
			...
			var qAnswerList = [];
			qAnswerList = $("#q_answer").val().split(",");
			
			console.log(qAnswerList.toString());
			
			var answerList = [];
			
			$('input:checkbox[name=answerMc]:checked').each(function () {
				//alert($(this).val());
			...
			if(qAnswerList !=""){
				if($("#answerType").val()=="2"){
					var mCnt = 0;
					for(var i=0;i<qAnswerList.length; i++){
						for(var j=0;j<answerList.length; j++){
							console.log("fff");
							if(qAnswerList[i] == answerList[j]){
								console.log(mCnt);
								mCnt = mCnt +1;
								break;
							}
					...
					if(mCnt == qAnswerList.length && qAnswerList.length == answerList.length){
						$("#answer").val(answerList.toString());
						alert("정답입니다.");
						$("#rowNum").val((parseInt($("#rowNum").val()))+1);
						$.saveResult();
					}else{
							...
							str +="</div>";
							    ...
							    if(data.sdResultInfo.fileYn == 'Y'){
							    	...
							    	if(data.sdResultInfo.question !=""){
									    str +="<li class='ly_cell1' id='questionText'>Q."+data.sdResultInfo.questionNum+"  "+data.sdResultInfo.question+"</br></li>";
									    if(data.sdResultInfo.answerType !="" && data.sdResultInfo.answerType != null){
										    if(data.sdResultInfo.answerType == "1"){
										    	...
										    	str +="<li class='ly_cell1 ck_button'>";
										    	for(var i =0; i<data.mcList.length; i++){
										    		str += "<label><input type='checkbox' id ='answerMc' name='answerMc' value='"+data.mcList[i].exNum+"'";
										    		var answerArr =[];
										    		answerArr = data.sdResultDetailInfo.answer.split(",");
										    		if(answerArr.length >0){
										    			for(var k=0; k<answerArr.length; k++){
										    				if(data.mcList[i].exNum == answerArr[k]){
												    			str +="checked='checked'";
												    		}
										    			}
										    		}
...
<!-- S : contents -->
    ...
    <div id="sdDiv">
	    ...
	    <div class="ly_block6">
		    ...
		    <div class="security_box">
		    	<div id="infoText" class="security_cell1">
		        	${sdResultInfo.infoText }<br/>
		        </div>
		        <table cellpadding="0" cellspacing="0">
		        	<tr>
		            	<th>점검 사항</th>
    	...
    	<div class="subTT"><span>SECURITY DAY</span> <a onclick="$.close()" style="margin-top: 10px; margin-right: 10px; float: right; cursor: pointer;"><img alt="" src="/img/ic_delete3.png"></a></div>
	    	...
	    	<div id="questionDiv" style="display: none" >
		    	<div class="ly_box1">
			        ...
			        <li class="ly_cell1 ck_button">
			        	<c:forEach var="result" items="${mcList}" varStatus="status">
			            <label><input type="checkbox"  id ="answerMc" name='answerMc' value='${result.exNum}'/> <span>${result.exNum}. ${result.exText}</span></label><br/>
			            </c:forEach>
			        </li>
			        </c:if>
			    </div>
