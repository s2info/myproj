...
$(function () {		
	...
	$('.btn_update').click(function(){
		...
		document.listForm.action = "/man/noticeRegisterPage.do";
		document.listForm.submit();
...
function searchList(page)
{
	...
	document.listForm.action = "<c:url value='/man/noticelist.do'/>";
	document.listForm.submit();
}
</script>
        ...
        <!-- E :search -->
    	 <div class="sch_view">
            ...
            <table cellpadding="0" cellspacing="0" class="tbl_list1">
				...
				<tr style="font-weight:bold;cursor:pointer;" class='btn_view' sqno="<c:out value="${result.sq_no}" />">
					<td><c:out value="${result.sq_no}" /></td>
					<td style="text-align:left;padding-left:15px;"><c:if test="${result.is_new eq 'Y' && result.read_count <= 0 }"><span style="line-height:50px;"><img style="vertical-align: middle;" src="/img/icon_new.png"> </span></c:if>${fn:substring(result.title, 0, 50)}<c:if test="${fn:length(result.title) > 50}" >...</c:if></td>
            ...
            <div class="btn_borderWrite">
            	<a class="btn_black" href='/man/noticeRegisterPage.do'><span>등록</span></a>
            </div>
            </c:if>
        </div>
