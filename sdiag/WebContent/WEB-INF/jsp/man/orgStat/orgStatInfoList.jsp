...
$(function () {		
	...
	$('#selAll_checkbox').click(function(){
		...
		$('input:checkbox[name=c_statSeq]').prop('checked',ischeck);

	});
	$(".goForm").click(function(){
		$("#formMod").val("I");
		
		document.listForm.action = "<c:url value='/man/orgStatPolForm.do'/>";
		document.listForm.method="post";
		document.listForm.submit();
	});
	
	$(".btn_view").click(function(){
		$("#statSeq").val($(this).attr('statSeq'));
		$("#formMod").val("U");
	...
	$(".btn_delete").click(function(){
		var checked_length =  $('input:checkbox[name=c_statSeq]:checked').length;
		if(checked_length <= 0){
			alert('삭제할 통계를 선택해 주세요.');
			return false;
		}else{
			...
			var statSeqList = [];
			$('input:checkbox[name=c_statSeq]:checked').each(function () {
				//alert($(this).val());
				statSeqList.push($(this).val());
			});
			var data = {
					statSeqList:statSeqList.toString()
			}
	...
	function goPage(page) {
	    ...
	    document.listForm.action = "<c:url value='/man/statInfoList.do'/>";
	    document.listForm.submit();
	}
    	 ...
    	 <div class="sch_view">
            ...
            <table cellpadding="0" cellspacing="0" class="tbl_list1">
				...
				<tr style="font-weight:bold; ">
					<td class="ck_button"><label><input type="checkbox" name="c_statSeq" value="<c:out value="${result.statseq}" />"><span></span></label></td>
					<td style="cursor: pointer;text-align: left; " class='btn_view' statSeq="<c:out value="${result.statseq}" />">${result.subject }</td>
					<%-- <td>${result.updtEmpNm }</td> --%>
					<td>${result.updtdate }</td>
					<td>
					<c:if test="${result.sumyn eq 'Y' }">
						<a href="javascript:$.goStatView('${result.statseq}')" class="btn_black"><span>통계보기</span></a>
					</c:if>
					</td>
				</tr>
				</c:forEach>
       	...
       	<input type="hidden" name="statSeq" id="statSeq" value="0" />
		</form:form>
    </div>
    <!-- E : center contents -->
    <!-- S : footer -->
