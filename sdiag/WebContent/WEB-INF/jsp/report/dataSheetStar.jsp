...
$(function () {		
    ...
    {
    	var polcheckedlist = getPolicyCheckedList();
    	...
    	var data = {begin_date:begin_date
    			...
    			,org_upper:org_upper
    			,auth:auth
    			,polcheckedlist:polcheckedlist};
    	//alert(JSON.stringify(data));
        $.ajax({
			//url : '/report/getdatareportcontents.do',  //날짜별 조회
			url : '/report/getdatareportcontentsex.do',  //정책별 조회
			data : data,
			type : 'POST',
			dataType : 'json',
    ...
    function getTableRowContents(tbl){
    	var body_list_item = {};
    	$(tbl).each(function(){
    		var collect_date = $(this).attr('collect_date');
    		alert(collect_date);
        	...
        	//alert(contents1_item.toString());
        	body_list_item.push(contents1_item);
    	});
    	
		return body_list_item.toString();
    }
    
    function getPolicyCheckedList(){
    	var polcheckedlist = [];
    	$('input:checkbox[name=selpolicylist]:checked').each(function(){
    		polcheckedlist.push($(this).val());
    	});
    	return polcheckedlist.toString();
    }
	...
	$('.btn_export_excel').click(function(){
	    ...
	    params[4] = JSON.stringify({"inputname": 'polcheckedlist',"inputvalue": polcheckedlist});
	   // params[4] = JSON.stringify({"inputname": 'content_body1',"inputvalue": getTableRowContents($('.contents1'))});
        gopage('/report/datareportexportexcelex.do', params, 'searchVO');
...
<div id="cnt">
    ...
    <form:form commandName="searchVO" name="listForm" method="post">
    	<div class="subTT"><span>기간별 데이터 추출</span></div>
    	<!-- S :search -->
        <div class="sch_block3">
	        ...
	        <li>
	        	<p>정책선택</p>
	        	 <c:forEach var="item" items="${diagItemList}" varStatus="status">
                   <input type="checkbox" name="selpolicylist" value="${item.polcode }" checked>${item.poldesc } 
                </c:forEach>
	        </li>
	        <br><br>          
       	...
       	<div class="sch_view nomal_list" style='display:'>
            <!-- S :list -->
            <div class='list_contents_body' style='margin-bottom:20px;'>
            </div>
        </div>  
