...
{ 
	...
	this.config = {
		...
		// The gauge value set by the user. Might underflow/overflow the min/max and must be adopted to be displayed correct. (precalculatedMax - precalculatedMin) / 2.0
		originalValue: (params.value) ? params.value : 0,
		
		// The current gauge value. Might be adopted to prevent underflow/overflow the min/max. (precalculatedMax - precalculatedMin) / 2.0
...
if (this.config.showGaugeShadow == true)
{
	...
	// Draw handy action range.
	if (this.config.lowerActionLimit > this.config.min && this.config.lowerActionLimit < this.config.max)
	{
		this.gaugeHandy = document.createElementNS(svgns, 'path');
		this.gaugeHandy.setAttributeNS(null, 'stroke', "none");
		this.gaugeHandy.setAttributeNS(null, 'fill',   'green');
		this.gaugeHandy.setAttributeNS(null, 'd',      this.calculateArcPath(this.config.min, this.config.lowerActionLimit, this.config.min, this.config.max,
																		   this.config.canvasW, this.config.canvasH,
																		   this.config.offsetX, this.config.offsetY, this.config.gaugeWidthScale));
		this.canvas.appendChild(this.gaugeHandy);
	}	
	...
	this.gaugeVAL.setAttributeNS(null, 'style',  "font-family:Malgun Gothic,Dotum,sans-serif,Helvetica,sans-serif,Arial,Verdana; font-size:" + Math.floor(this.config.canvasW / 8)*1.5 + "px; font-weight:bold; fill-opacity:1.0; fill:" + this.config.valueColor + "; text-anchor:middle;");
