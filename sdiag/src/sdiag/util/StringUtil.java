...
public class StringUtil {
	...
	public static boolean nullCheck(Object src){
	 ...
	 * @return
	 */
	public static String[] delStringArray(String[] arr, int deleteKey) {
		if (arr == null){
			return null;
		}
		
		String[] reArr = new String[arr.length - 1];
		int cnt = 0;
	 ...
	 * @return
	 */
	public static String[] createStringArray(String[] arr, int size){
		if (arr == null){
			arr = new String[size];
		}
	 ...
	 * @return
	 */
	public static String[] inStringArray(String value, int size) {
	...
	public static String arrayToString(String[] strArray, int matchCount) throws Exception{
		String str = "";
		if (strArray != null && strArray.length > 0) str = strArray[matchCount];
		return str;
	}
	
	/**
	 * 날짜타입 xxxx-xx-xx 형식을 xxxxxxxx로 바꿈 입력값이 없다면 ""빈값을 리턴
	 * @return
	 * @throws Exception
	 */
	public static String dateTypeSubstring(String date, String dmt) throws Exception{
		if (date == null || date.equals("")){
			return "";
		}
		
		String[] dateArray = date.split(dmt);
		if (dateArray.length > 0){
			int length = dateArray.length;
			date = "";
			for (int i = 0 ; i < length; i++) date += dateArray[i];
		}
		else{
			return "";
		}
	...
	public static List<EgovMap> doubleToLong(List<EgovMap> list, String key){
	...
	public static boolean hashMapContainsKey(HashMap<String,?> map,
												String key){
		Iterator<String> itr = map.keySet().iterator();
		while(itr.hasNext()){
			String itrKey = itr.next();
			if ((itrKey.substring(0, 2)).equals(key)) return true;
		}
		return false;
	}
